...
import { useGroupList } from "@/hooks/api_hooks/auth.queries";
import type { GroupInfo } from "@/lib/auth/types/auth.types";
import { ShieldCheck, Users, Eye, Code2, Layers } from "lucide-react";
...
export interface GroupSelectDropdownProps
...
export default function GroupSelectDropdown({
  ...
  const filteredGroups: GroupInfo[] = (groups ?? []).filter(
    (g) => !excludeSlugs.includes(g.slug)
  );
