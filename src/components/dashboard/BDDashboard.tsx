...
import type { BDMember, LeadLogEntry, UpworkProfile, User } from "@/types/types";
import {
  Award,
  DollarSign,
  ExternalLink,
...
export default function BDDashboard({ leads, members, profiles, user }: Props) {
  ...
  const recentLeads = [...filteredMyLeads]
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }} className="animate-fade-in">
              ...
              <tbody>
                {myHotLeads.map((lead, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "8px 12px", color: "var(--text-muted)", maxWidth: 160 }}>
                      <span
                        style={{
                          overflow: "hidden",
                          textOverflow: "ellipsis",
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 3 }}>
            You've submitted <strong style={{ color: "#818cf8" }}>{activity.monthlyBids}</strong>{" "}
            bids this month — target is{" "}
            <strong style={{ color: "#22d3ee" }}>{monthly_target}</strong>
          </div>
        </div>
        <div style={{ textAlign: "center" }}>
          <div
            style={{
              fontSize: 22,
              fontWeight: 700,
              color: targetCompletion >= 1 ? "#10b981" : "#f59e0b",
            }}
          >
          ...
          >
            <div
              style={{
                height: "100%",
                width: `${Math.min(targetCompletion * 100, 100)}%`,
                background: targetCompletion >= 1 ? "#10b981" : "#f59e0b",
                borderRadius: 2,
              }}
            />
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 14 }}>
        ...
        <KPICard
          ...
          subtitle={`${pipeline.activeLeadsCount} active`}
        ...
        <KPICard
          title="Connects Used"
          value={activity.totalConnectsUsed}
      ...
      {myLeads.length > 0 && (
        <Card className={cardStyle}>
          <SectionHeader
            title="My Engagement Mix"
            subtitle="Your Fixed-price vs Hourly breakdown"
          />
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            ...
            >
              ...
              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 3 }}>
                Win: {formatPercent(myEng.fixedWinRate)} · Avg:{" "}
                {formatCurrency(myEng.avgFixedValue)}
              </div>
            </div>
            <div
              style={{
            ...
            >
              ...
              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 3 }}>
                Win: {formatPercent(myEng.hourlyWinRate)} · Avg:{" "}
                {formatCurrency(myEng.avgHourlyValue)}
              </div>
            </div>
            <div
              style={{
            ...
            >
              ...
              <div style={{ fontSize: 28, fontWeight: 800, color: "#10b981" }}>
                {myEng.hourlyCount > 0 ? `$${myEng.avgHourlyRate.toFixed(0)}` : "—"}
                <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                  {myEng.hourlyCount > 0 ? "/hr" : ""}
                </span>
              </div>
              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 3 }}>
                Avg est. {myEng.avgEstimatedHours.toFixed(0)}h per engagement
              </div>
            </div>
            <div
              style={{
            ...
            >
              ...
              <div style={{ fontSize: 28, fontWeight: 800, color: "#f59e0b" }}>
                {myEng.totalHourlyHours.toLocaleString()}
                <span style={{ fontSize: 12, color: "var(--text-muted)" }}>h</span>
              </div>
              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 3 }}>
                {formatCurrency(myEng.hourlyPipelineValue)} active hourly value
              </div>
            </div>
          </div>
        </Card>
      ...
      {myAvgBidsRow && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <Card className={cardStyle}>
            <SectionHeader
              title="My Daily Bidding Pace"
              subtitle="Average bids submitted per active day"
            />
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
              <div
                style={{
              ...
              >
                ...
                <div style={{ fontSize: 28, fontWeight: 700, color: "#818cf8" }}>
                  {myAvgBidsRow.avgBidsPerDay.toFixed(1)}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                <div style={{ fontSize: 28, fontWeight: 700, color: "#22d3ee" }}>
                  {myAvgBidsRow.avgLast30.toFixed(1)}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                <div
                  style={{
                    fontSize: 28,
                    fontWeight: 700,
                    color: myAvgBidsRow.last7DaysBids >= 5 ? "#10b981" : "#f59e0b",
                  }}
                >
                  {myAvgBidsRow.last7DaysBids}
                </div>
              </div>
              <div
                style={{
          ...
          <Card className={cardStyle}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 4 }}>
                  ...
                  <span
                    style={{
                      fontSize: 12,
                      fontWeight: 600,
                      color: wow.bidsDelta >= 0 ? "#10b981" : "#ef4444",
                    }}
                  >
                    {wow.bidsDelta >= 0 ? "+" : ""}
                    {wow.bidsDelta.toFixed(0)}% vs last week ({wow.lastWeekBids})
                  </span>
                </div>
              </div>
              <div
              ...
              >
                ...
                <div style={{ fontSize: 30, fontWeight: 700, color: "#10b981" }}>
                  {wow.thisWeekWins}
                </div>
                <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>
                  Last week: <strong style={{ color: "var(--text)" }}>{wow.lastWeekWins}</strong>{" "}
                  wins
                </div>
              </div>
              <div
              ...
              >
                ...
                <div style={{ fontSize: 20, fontWeight: 700, color: "var(--text)", marginTop: 2 }}>
                  {formatPercent(cost.boostedWinRate)}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                <div style={{ fontSize: 20, fontWeight: 700, color: "var(--text)", marginTop: 2 }}>
                  {formatPercent(cost.normalWinRate)}
                </div>
              </div>
            </div>
          </Card>
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding: "9px 10px", color: "#22d3ee", fontWeight: 600 }}>
                    ...
                    {lead.engagement_type === "Hourly" &&
                      lead.hourly_rate &&
                      lead.estimated_hours && (
                        <div style={{ fontSize: 10, color: "var(--text-muted)" }}>
                          ${lead.hourly_rate}/hr × {lead.estimated_hours}h
                        </div>
                      )}
                  </td>
                  ...
                  <td style={{ padding: "9px 10px" }}>
                    <span
                      style={{
                        ...
                        borderRadius: 4,
                        background:
                          lead.bid_type === "Boosted"
                            ? "rgba(245,158,11,0.15)"
                            : "rgba(99,102,241,0.15)",
                        color: lead.bid_type === "Boosted" ? "#f59e0b" : "#818cf8",
                        border: `1px solid ${lead.bid_type === "Boosted" ? "rgba(245,158,11,0.3)" : "rgba(99,102,241,0.3)"}`,
                      }}
                    >
                      {lead.bid_type}
                    </span>
