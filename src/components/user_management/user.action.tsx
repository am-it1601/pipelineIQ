...
import {
    ...
    useUnbanUser,
} from "@/hooks/http/auth/auth.mutations";
...
const UserActionMenu = ({ user }: { user: UserWithDetails }) => {
    ...
    const handleToggleDisable = () => {
        ...
        action.mutate(user.id, {
            onSuccess: () => toast.success(isDisabled ? "User enabled" : "User disabled"),
    ...
    const handleResetMfa = () => {
        resetMfa.mutate(user.id, {
            onSuccess: (data) => toast.success(`MFA reset — ${data.deletedCount} factor(s) removed`),
            onError: (err) => toast.error(err.message),
        });
    };
