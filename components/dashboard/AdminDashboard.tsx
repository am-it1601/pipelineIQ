...
import DashboardFilter, {
  DEFAULT_FILTERS,
  DashboardFilters,
} from "@/components/ui/DashboardFilter";
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }} className="animate-fade-in">
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
            ...
            <span style={{ fontSize: 12, color: "var(--text-muted)", marginLeft: 4 }}>
              {hotLeads.length} starred across the team
            </span>
          </div>
              ...
              <tbody>
                {hotLeads.map((lead, i) => (
                  <tr
                    key={lead.id}
                    style={{
                      background: i % 2 === 0 ? "transparent" : "rgba(245,158,11,0.03)",
                    ...
                    onMouseLeave={(e) =>
                      (e.currentTarget.style.background =
                        i % 2 === 0 ? "transparent" : "rgba(245,158,11,0.03)")
                    }
                  >
                    <td
                      style={{
                    ...
                    <td style={{ padding: "8px 12px", color: "var(--text)", whiteSpace: "nowrap" }}>
                      {members.find((m) => m.id === lead.assigned_to_id)?.full_name || "—"}
                    </td>
                    <td style={{ padding: "8px 12px" }}>
                      <span
                        style={{
                          ...
                          borderRadius: 4,
                          background:
                            lead.engagement_type === "Hourly"
                              ? "rgba(34,211,238,0.12)"
                              : "rgba(99,102,241,0.12)",
                          color: lead.engagement_type === "Hourly" ? "#22d3ee" : "#818cf8",
                          border: `1px solid ${lead.engagement_type === "Hourly" ? "rgba(34,211,238,0.3)" : "rgba(99,102,241,0.3)"}`,
                        }}
                      >
        ...
        <KPICard
          ...
          value={formatPercent(
            conversion.winRate > 0
              ? (conversion.wonCount + conversion.wonCount * 2) / activity.totalBids
              : 0
          )}
          subtitle="viewed / submitted"
          accentColor="var(--accent)"
        />
        <KPICard
          title="Boosted Win Rate"
          value={formatPercent(cost.boostedWinRate)}
          subtitle={`${cost.boostedBidCount} boosted bids`}
          accentColor="var(--warning)"
        />
        <KPICard
          title="Won Revenue"
          value={formatCurrency(pipeline.wonValue)}
          subtitle="total closed value"
          icon={<DollarSign size={16} />}
          accentColor="var(--success)"
          highlight
        />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 16 }}>
        ...
        <Card className={cardStyle}>
          ...
          >
            <div>
              <div style={{ fontSize: 10, color: "var(--text-muted)" }}>Discussion → Win</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#10b981" }}>
                {formatPercent(conversion.discussionToWinRate)}
              </div>
            </div>
            <div>
              <div style={{ fontSize: 10, color: "var(--text-muted)" }}>Bids Per Win</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#f59e0b" }}>
                {conversion.bidsPerWin}
              </div>
            </div>
          </div>
        </Card>
            ...
            <tbody>
              {bdPerf.map((row, i) => (
                ...
                >
                  ...
                  <td style={{ padding: "10px 12px", color: "#22d3ee", fontWeight: 600 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      >
                        <div
                          style={{
                            height: "100%",
                            width: `${Math.min(row.targetCompletion * 100, 100)}%`,
                            background:
                              row.targetCompletion >= 1
                                ? "#10b981"
                                : row.targetCompletion >= 0.6
                                  ? "#f59e0b"
                                  : "#ef4444",
                            borderRadius: 3,
                          }}
                      ...
                      >
                        {formatPercent(row.targetCompletion, 0)}
                      </span>
                    </div>
                  </td>
                </tr>
              ...
              {profilePerf.map((row, i) => (
                ...
                >
                  ...
                  <td
                    style={{
                      padding: "8px 10px",
                      color: row.winRate >= 0.15 ? "#10b981" : "#f59e0b",
                      fontWeight: 600,
                    }}
                  >
                    {formatPercent(row.winRate)}
                  </td>
                  <td style={{ padding: "8px 10px", color: "#22d3ee" }}>
                    {formatCurrency(row.pipelineValue)}
                  </td>
                  <td style={{ padding: "8px 10px", color: "var(--text-muted)" }}>
                    {row.boostedBids} / {row.normalBids}
                  </td>
                </tr>
              ))}
            </tbody>
              ...
              <div key={s.source}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  ...
                  <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
                    <span style={{ color: "#10b981", fontWeight: 600 }}>{s.won} won</span> /{" "}
                    {s.total} bids
                  </div>
                </div>
                <div
                ...
                >
                  <div
                    style={{
                      height: "100%",
                      width: `${(s.total / leads.length) * 100}%`,
                      background: COLORS[i],
                      borderRadius: 3,
                    }}
                  />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              >
                ...
                <div style={{ fontSize: 20, fontWeight: 700, color: "var(--text)", marginTop: 4 }}>
                  {cost.boostedBidCount}
                </div>
                <div style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  Win: {formatPercent(cost.boostedWinRate)}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                <div style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  Win: {formatPercent(cost.normalWinRate)}
                </div>
              </div>
            </div>
          </div>
        ...
        const engChartData = [
          {
            name: "Fixed",
            bids: eng.fixedCount,
            pipeline: Math.round(eng.fixedPipelineValue / 1000),
            winRate: Math.round(eng.fixedWinRate * 100),
          },
          {
            name: "Hourly",
            bids: eng.hourlyCount,
            pipeline: Math.round(eng.hourlyPipelineValue / 1000),
            winRate: Math.round(eng.hourlyWinRate * 100),
          },
        ];
        return (
          <div>
            <SectionHeader
              title="Engagement Mix Analysis"
              subtitle="Fixed-price vs Hourly breakdown across all bids"
            />
            {/* 4 KPI cards */}
            <div
              style={{
            ...
            >
              <Card className={cardStyle} style={{ borderTop: "3px solid var(--primary)" }}>
                ...
                <div
                  style={{ fontSize: 28, fontWeight: 800, color: "var(--text)", margin: "6px 0" }}
                >
                  {eng.fixedCount}
                </div>
                <div style={{ fontSize: 12, color: "var(--primary)" }}>
                  {(eng.fixedShare * 100).toFixed(0)}% of all bids
                </div>
                <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>
                  Win rate: {formatPercent(eng.fixedWinRate)}
                </div>
                <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
                  Avg value: {formatCurrency(eng.avgFixedValue)}
                </div>
              </Card>
              <Card className={cardStyle} style={{ borderTop: "3px solid var(--accent)" }}>
                ...
                <div style={{ fontSize: 12, color: "var(--accent)" }}>
                  {(eng.hourlyShare * 100).toFixed(0)}% of all bids
                </div>
                <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>
                  Win rate: {formatPercent(eng.hourlyWinRate)}
                </div>
                <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
                  Avg value: {formatCurrency(eng.avgHourlyValue)}
                </div>
              </Card>
              <Card className={cardStyle} style={{ borderTop: "3px solid var(--success)" }}>
                ...
                >
                  ${eng.avgHourlyRate.toFixed(0)}
                  <span style={{ fontSize: 14, color: "var(--text-muted)", fontWeight: 500 }}>
                    /hr
                  </span>
                </div>
                <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
                  Across {eng.hourlyCount} hourly bids
                </div>
                <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>
                  Avg est. hours: {eng.avgEstimatedHours.toFixed(0)}h
                </div>
              </Card>
              <Card className={cardStyle} style={{ borderTop: "3px solid var(--warning)" }}>
                ...
                >
                  {eng.totalHourlyHours.toLocaleString()}
                  <span style={{ fontSize: 14, color: "var(--text-muted)", fontWeight: 500 }}>
                    hrs
                  </span>
                </div>
                <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
                  Active hourly engagements
                </div>
                <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>
                  Hourly pipeline: {formatCurrency(eng.hourlyPipelineValue)}
                </div>
              </Card>
            </div>
            ...
            <Card className={cardStyle}>
              <div
                style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", marginBottom: 14 }}
              >
                Fixed vs Hourly — Bids, Pipeline ($K) &amp; Win Rate (%)
              </div>
              <ResponsiveContainer width="100%" height={200}>
