...
export default function ExtendedKPISections({ leads, members, profiles }: Props) {
  ...
  const avgBids = getAvgBidsPerDay(leads, members.filter(m => m.status === 'active'));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <Card className={cardStyle}>
          ...
          {avgBids.map((row, i) => (
            <div key={row.memberId} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: MEMBER_COLORS[i % MEMBER_COLORS.length], display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#fff' }}>
                  {row.memberName.split(' ').map(n => n[0]).join('')}
                </div>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{row.memberName.split(' ')[0]}</span>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <div>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Avg/Day</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: MEMBER_COLORS[i % MEMBER_COLORS.length] }}>
                    {row.avgBidsPerDay.toFixed(1)}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Avg/Day (30d)</div>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>{row.activeDays}</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>Last 7d</div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: row.last7DaysBids >= 5 ? '#10b981' : '#f59e0b' }}>{row.last7DaysBids}</div>
                </div>
              </div>
            </div>
          ))}
        ...
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={avgBids} barGap={4}>
            ...
            <XAxis dataKey="memberName" tickFormatter={(v) => v.split(' ')[0]} tick={{ fontSize: 11, fill: 'var(--text-muted)' }} axisLine={false} tickLine={false} />
      ...
      <Card className={cardStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(memberNames.length, 4)}, 1fr)`, gap: 10, marginTop: 16 }}>
          {(() => {
            ...
            return memberNames.map((name, i) => {
              ...
              return (
                <div key={name} style={{ padding: '10px 12px', background: 'var(--surface-2)', borderRadius: 4, border: '1px solid var(--border)' }}>
                  <div style={{ fontSize: 11, color: MEMBER_COLORS[i], fontWeight: 600 }}>{name}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                    <Icon size={14} color={color} />
                    <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)' }}>{thisW} bids</span>
                    <span style={{ fontSize: 11, color }}>
                      {delta > 0 ? '+' : ''}{delta.toFixed(0)}%
                    </span>
                  </div>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>vs prior week ({lastW})</div>
                </div>
              );
            });
          })()}
            ...
            <thead>
              <tr style={{ background: 'var(--surface-2)' }}>
                {[
                  ...
                  'Momentum', 'Avg Proposal', 'Pipeline', 'Boosted', 'Normal',
                ].map(h => (
              ...
              {profileMomentum.map((row, i) => {
                ...
                return (
                  <tr key={row.profileId}
                    ...
                    style={{ background: isHovered ? 'rgba(15, 108, 189, 0.05)' : i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)', transition: 'background 0.12s', cursor: 'default' }}>
                    ...
                    <td style={{ padding: '10px 12px', color: 'var(--text-muted)' }}>{row.normalBids}</td>
                  </tr>
                );
              })}
            </tbody>
